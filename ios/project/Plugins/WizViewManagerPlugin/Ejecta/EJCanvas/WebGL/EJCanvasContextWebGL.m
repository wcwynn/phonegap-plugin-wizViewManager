...
- (id)initWithWidth:(short)widthp height:(short)heightp {
	if( self = [super init] ) {
		glContext = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2 sharegroup:[WizCanvasView instance].glSharegroup];
...
- (void)create {
	...
	CGSize screen = [WizCanvasView instance].view.bounds.size;
	...
	[WizCanvasView instance].internalScaling = internalScaling;
	...
	[[WizCanvasView instance] hideLoadingScreen];
	[[WizCanvasView instance].view addSubview:glview];
}
