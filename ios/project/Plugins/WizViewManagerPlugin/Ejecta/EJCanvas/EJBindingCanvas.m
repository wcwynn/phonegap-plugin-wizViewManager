...
- (void)dealloc {
	...
	if( jsCanvasContext ) {
		JSValueUnprotect([WizCanvasView instance].jsGlobalContext, jsCanvasContext);
	}
	[super dealloc];
}
...
EJ_BIND_SET(width, ctx, value) {
	...
	if( renderingContext ) {
		[WizCanvasView instance].currentRenderingContext = renderingContext;
...
EJ_BIND_FUNCTION(getContext, ctx, argc, argv) {
	...
	[WizCanvasView instance].currentRenderingContext = nil;
	...
	if( newContextMode == kEJCanvasContextMode2D ) {
		if( isScreenCanvas ) {
			...
			[WizCanvasView instance].screenRenderingContext = sc;		
			renderingContext = sc;
		}
		else {
